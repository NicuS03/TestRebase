...
2. Builds gt using cmake
3. Creates a `.deb` package with debhelper
4. Generates source package files (`.orig.tar.gz`, `.debian.tar.xz`, `.dsc`)
...
```bash
./build-gt-deb.sh [version] [architecture]
```
...
# Build version 1.0 for 64-bit
./build-gt-deb.sh 1.0 amd64

# Build version 1.0 for 32-bit x86
./build-gt-deb.sh 1.0 i386

# Build version 2.0 for 64-bit ARM
./build-gt-deb.sh 2.0 arm64

# Build version 1.0 for 32-bit ARM
./build-gt-deb.sh 1.0 armhf
```
